...
function hitung(nilai) {
    ...
    if (nilai === "=") {
        ...
        try {
            // menangkap kalau di output masih kosong gabakal bisa di hasilkan dengan memencet sama dengan
            if (output.innerText === "") return;

            // mencegah 21+ bisa di enter 
            if (tandaHitung.includes(output.innerText.slice(-1))) {
                notif()
                return;
            }
